...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.epsilon = math.exp(-0.0038*self.trial)  #custom decay function
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if state not in self.Q.keys():
                self.Q[state] = defaultdict(float)
                for act in self.env.valid_actions:                 
            ...
            if self.epsilon > random.random():
                ...
                for act in self.env.valid_actions:
                    if self.Q[state][act] == maxval:
                        possible_actions.append(act)  #when there are more than 1 act with max value
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[state][action] = reward*self.alpha + currentQ*(1-self.alpha)  #gamma = 0
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning=True, alpha = 0.5, epsilon=0.8)
